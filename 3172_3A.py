...
# st.markdown(f"""
#     <div style="display: flex; justify-content: center;">
#         <img src="data:image/png;base64,{img_b64}" style="max-width: 100%; width: 1400px;">
#     </div>
# """, unsafe_allow_html=True)
...
option = st.selectbox(label='tf', options=tf_names) 
imgfile = f"./data/{sample}/spatial_tf/{option}_(TF_activity_and_mRNA_expression).png"
img_b64 = base64.b64encode(Path(imgfile).read_bytes()).decode()
st.markdown(f"""
    <div style="width: 100%;">
        <img src="data:image/png;base64,{img_b64}" style="width: 800px;">
    </div>
""", unsafe_allow_html=True)

st.write("")

option = st.selectbox(label='cell type', options=ct_names) 
imgfile = f"./data/{sample}/Lee_TF_Marker_Figs/{option}_Lee's_L_between_TF_Marker.png"
img_b64 = base64.b64encode(Path(imgfile).read_bytes()).decode()
st.markdown(f"""
    <div style="width: 100%;">
        <img src="data:image/png;base64,{img_b64}" style="width: 1400px;">
    </div>
""", unsafe_allow_html=True)
